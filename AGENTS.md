...
**关键规则：Astro 从 frontmatter 的 `title:` 字段渲染文章标题。绝对不要在 Markdown 正文的开头写 `# 标题`。正文应直接从内容或 `## 二级标题` 开始。**
...
# 文章标题   ← 错误：Astro 已经从 frontmatter 渲染了标题，这里是重复的
...
日报的 `# 日报 YYYY-MM-DD` 标题已经由 frontmatter `title:` 字段提供，正文中的同名一级标题是多余的。日报正文应直接从编号条目开始。
...
# 日报 2025-01-15   ← 错误：与 frontmatter title 重复
